...
export interface NativeBindings {
  ...
  configSetStorageTuning(config: Handle, pageSize: number, threshold: number, groupCommit: number): void;
...
async function createBunBindings(libPath: string): Promise<NativeBindings> {
  ...
  return {
    engineOpen: (path) => symbols.fl_engine_open(Buffer.from(path + '\0')),
    engineOpenWithConfig: (path, config) => symbols.fl_engine_open_with_config(Buffer.from(path + '\0'), config),
    ...
    configSetEncryptionKey: (c, k) => symbols.fl_config_set_encryption_key(c, k ? Buffer.from(k + '\0') : null),
    configSetAudit_log: (c, e, p) => symbols.fl_config_set_audit_log(c, e, p ? Buffer.from(p + '\0') : null),
    configSetAuditLog: (c, e, p) => symbols.fl_config_set_audit_log(c, e, p ? Buffer.from(p + '\0') : null),
    ...
    configSetStorageTuning: (c, ps, th, gc) => symbols.fl_config_set_storage_tuning(c, ps, 0, th, gc),
    ...
    engineWatch: (engine, collection, callback) => {
      ...
      return symbols.fl_engine_watch(engine, Buffer.from(collection + '\0'), cb, null);
    },
    ...
    docInsertInt: (doc, key, value) => symbols.fl_doc_insert_int(doc, key, BigInt(value)),
    ...
    queryWhereEqInt: (query, field, value) => symbols.fl_query_where_eq_int(query, field, BigInt(value)),
...
async function createNodeBindings(libPath: string): Promise<NativeBindings> {
  ...
  // New Opaque handles
  const FL_Config = koffi.opaque('FL_Config');
  const FL_Watch = koffi.opaque('FL_Watch');
  const FL_Engine = koffi.opaque('FL_Engine');
  const FL_Doc = koffi.opaque('FL_Doc');
  const FL_Batch = koffi.opaque('FL_Batch');
  const FL_Query = koffi.opaque('FL_Query');
  ...
  return {
    ...
    engineWatch: (engine, collection, callback) => {
      // Create a persistent callback pointer for Koffi
    ...
    docInsertBin: (doc, key, bytes) => fn.fl_doc_insert_bin(doc, key, bytes, bytes.byteLength),
