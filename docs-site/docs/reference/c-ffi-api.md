---
title: C-FFI API (Complete v0.5.10 Surface)
---

This page documents the full flat C interface in `include/firelite.h`.

## Ownership and return conventions

- Most mutating functions return `0` on success, non-zero on error.
- On error, call `fl_last_error()` for thread-local error text.
- Any `char*` returned from FireLite must be released with `fl_string_free(...)`.
- Handle types are opaque and must be released with matching free functions.
...
FL_Engine *fl_engine_open_with_config(const char *path, FL_Config *config); // takes config ownership
void fl_engine_free(FL_Engine *engine);

FL_Config *fl_config_new(void);
void fl_config_free(FL_Config *config);
void fl_config_set_durability(FL_Config *config, int32_t mode); // 0=Always,1=Interval,2=Manual,3=OnCommit
...
int32_t fl_doc_insert_array(FL_Doc *doc, const char *key, FL_Array *array); // takes array ownership
...
int32_t fl_query_where_in(FL_Query *query, const char *field, FL_Array *array); // takes array ownership
...
```c
using FL_OnSnapshotCallback = void(*)(const char *collection, const char *path, int32_t kind, void *user_data);
